...
// CondoRole is the granular role inside a condomínio (only relevant for sindico-type accounts).
...
// User is an account on the platform: síndico (condo manager), prestador (service provider), or admin.
type User struct {
	...
	// Profile fields populated by the specific register endpoints.
	...
	Whatsapp     string `gorm:"size:20;column:whatsapp" json:"whatsapp,omitempty"`
